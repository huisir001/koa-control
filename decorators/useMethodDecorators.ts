...
export default () => {
    return {
        ...
        Bind: (Dto: Control.Dto) => {
            ...
            return (target: Control.Obj, propertyKey: string, desc: PropertyDescriptor) => {
                // 验证
                targetVerify(desc)
                // 缓存装饰器信息
 ...
 * 缓存装饰器信息
 */
function defineMethodDecMetadata(target: Control.Obj, propertyKey: string, methodDecKey: string) {
 ...
 */
function defineDataCach(target: Control.Obj, propertyKey: string, data: any, key: string): Control.Data {
 ...
 * 根据参数装饰器对方法参数进行重置
 */
function argumentsReset(target: Control.Obj, key: string, data: Control.Data) {
    // 获取参数装饰器元信息
 ...
 * 此装饰器必用
 */
function methodRestrict(m: 'GET' | 'POST' | 'PUT' | 'DELETE') {
    /**
     * 请求方式
