 ...
 * @Date: 2022-07-04 15:51:53
 * @LastEditTime: 2022-07-07 16:39:41
 */
 ...
 * 非属性使用验证，报错
 */
function targetVerify(target: Control.Obj, key: string) {
 ...
 * 缓存装饰器信息
 */
function definePropDecMetadata(target: Control.Obj, propertyKey: string, propDecKey: string) {
 ...
 */
function dataGrouping(group: 'body' | 'query' | 'params' | 'raw' | 'files' | 'cookie' | 'header') {
    return (target: Control.Obj, key: string): any => {
        targetVerify(target, key)
 ...
 * 类型绑定
 */
function typeBind(type: 'String' | 'Boolean' | 'Number' | 'Object' | 'Array') {
    return function (target: Control.Obj, key: string) {
        targetVerify(target, key)
