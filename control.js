...
import { CLASS_PREFIX_METADATA_KEY, CLASS_SIGN_METADATA_KEY, METHOD_PATH_METADATA_KEY } from "./common/constants.js";
const { e404 } = useResponseError();
/**
 * 控制器，含路由分发，无须再使用koa-router等路由管理中间件
 */
export default (app, { controller, bodyParserOpts }) => {
    // 路由中转
    const routerBus = new Router();
    // 缓存路径验证
    const fullRoutePathsTemp = [];
    // 验证控制器
